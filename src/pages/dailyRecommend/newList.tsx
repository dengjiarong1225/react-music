...
function Introduce() {
  return (
    <div className={styles.contentWrapperHeader}>
      <div className={styles.contentWrapperHeaderPadding}>
        <div className={styles.contentWrapperContext}>
          ...
          <button className={styles.contentButton}>
            <Link className={styles.contentButton} to="/">试听专辑</Link>
          </button>
        </div>
      </div>
...
function SongList() {
  ...
  return (
    <div className={styles.contentSection}>
      <div className={styles.contentSectionTitle}>最新歌曲</div>
      <ul className={styles.searchUl}>
        { data.result.map((item: SongItem, index: number) => (
          <li className={styles.adobeProduct} key={index}>
            <div className={styles.products}>{item.song.album.name}</div>
            <span className={styles.status}>
              <span className={[styles.statusCircle, styles.green].join(' ')}></span>{item.song.artists[0].name}</span>
            <span className={styles.status}>{computedDuration(item.song.duration)}</span>
            <span className={styles.status}>{item.song.alias[0]}</span>
            <div className={styles.buttonWrapper}>
              <div className={styles.clicksvg}>
                <svg className={styles.icon} viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2313" width="40" height="40">
              ...
              <div className={styles.clicksvg}>
                <svg className={styles.icon} viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3387" width="40" height="40">
                  <path
              ...
              <div className={styles.clicksvg}>
                <a href="/" target="_blank" download>
                  <svg className={styles.icon} viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4288" width="40" height="40">
...
function HotList() {
  ...
  return (
    <div className={styles.contentSection}>
      <div className={styles.contentSectionTitle}>热门歌单</div>
      <div className={styles.appsCard}>
        {data.playlists.map((item: { coverImgUrl: string | undefined; name: string | undefined; id: number }) =>  
          <div className={styles.block} key={item.id}>
              <img className={[styles.songlistimg, styles.appCard].join(' ')} src={item.coverImgUrl} />
              <h5 className={styles.songlisttext}>{item.name}</h5>
          </div>
        )}
      </div>
